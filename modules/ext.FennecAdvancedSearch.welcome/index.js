 ...
 *
 * This file is part of the 'ext.FennecAdvancedSearch.welcome' module.
 *
 * It is enqueued for loading on all pages of the wiki,
 * from FennecAdvancedSearchHooks::onBeforePageDisplay() in FennecAdvancedSearch.hooks.php.
 */
( function () {
	var welcome, dayMap;
	...
	welcome = {
		init: function () {
			...
			$box = $( '<div class="mw-welcome-bar"></div>' ).text(
				mw.user.isAnon() ?
					mw.msg( 'fennecadvancedsearch-welcome-title-loggedout', mw.msg( dayKey ) ) :
					mw.msg( 'fennecadvancedsearch-welcome-title-user', mw.user.getName(), mw.msg( dayKey ) )
			);
		...
		getColorByDate: function ( dayKey ) {
			var colors = mw.config.get( 'wgFennecAdvancedSearchWelcomeColorDays' );

			if ( Object.hasOwnProperty.call( colors, dayKey ) ) {
				return colors[ dayKey ];
			}

			return mw.config.get( 'wgFennecAdvancedSearchWelcomeColorDefault' );
		}
	};

	module.exports = welcome;

	mw.FennecAdvancedSearchWelcome = welcome;
}() );
